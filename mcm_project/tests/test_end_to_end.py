...
class TestEndToEndRecovery:
    ...
    def test_recover_noiseless_full_model(self, medium_scheme):
        """Noiseless data: perfect signal prediction; parameter recovery
        is limited by model identifiability (coupled f/R)."""
        true = MCMParameters(
        ...
        # Signal prediction must be excellent
        assert result["R2"] > 0.99
        # Orientation recovered accurately
        ang_err = np.degrees(np.arccos(min(1, abs(np.dot(true.mu, fitted.mu)))))
        assert ang_err < 5.0
        # Volume fractions in rough ballpark (identifiability-limited)
        assert fitted.f_ic == pytest.approx(true.f_ic, abs=0.20)
        assert fitted.f_ss + fitted.f_ls == pytest.approx(true.f_ss + true.f_ls, abs=0.15)

    def test_recover_low_snr_full_model(self, medium_scheme):
        """SNR=100: good signal prediction; parameter recovery has noise."""
        true = MCMParameters(
        ...
        ang_err = np.degrees(np.arccos(min(1, abs(np.dot(true.mu, fitted.mu)))))
        assert ang_err < 10.0
        # Total tissue volume fraction roughly correct
        assert fitted.f_ic + fitted.f_ss + fitted.f_ls + fitted.f_ec == pytest.approx(
            true.f_ic + true.f_ss + true.f_ls + true.f_ec, abs=0.20
        )
